...
while success:
    ...
    # Find bounding box around the motion
    non_zero_pixels = np.argwhere(heat_map > 0)
    if len(non_zero_pixels) > 0:
        x, y, w, h = cv2.boundingRect(non_zero_pixels)
        
        if w > 0 and h > 0:  # Check if the width and height are valid
            # Crop motion heatmap
            cropped_heatmap = heat_map[y:y+h, x:x+w]

            # Crop video feed using the same region
...
# Additional loop to keep the window open until 'q' is pressed
